    ...
    <section>
        ...
        <div class="contentBody"
            ...
            <el-table :data="tableData" border style="width: 100%">
                ...
                <el-table-column align="center" header-align='center' prop="orderNo" label="采购单号"></el-table-column>
                <el-table-column align="center" header-align='center' prop="name" label="采购单名称"></el-table-column>
                <el-table-column align="center" header-align='center' prop="dateTime" label="采购日期" width="100"></el-table-column>
...
<script type="text/ecmascript-6">
    import {requestDeletePurchase, requestSearchPurchase, requestStatusPurchase} from '../../api/api';
    ...
    export default {
        ...
        methods: {
            ...
            handleSizeChange(val){
                ...
                requestSearchPurchase(_this.param).then(res => {
            ...
            handleClick(index, row, doType) {
                ...
                if(doType == 'delete'){
                    this.$confirm('真的删除该行数据么, 是否继续?', '提示', {
                        ...
                        requestDeletePurchase(deleteParam).then(res => {
                            this.$message({
                                type: 'success',
                                message: '删除成功!',
                                duration:300,
                    ...
                    this.$router.push({ path: '/editPurchase'});
                }else if(doType == 'detail'){
                    sessionStorage.setItem('oId', row.id);
                    this.$router.push({ path: '/detailPurchase'});
                }else if(doType == 'edit'){
                    sessionStorage.setItem('oId', row.id);
                    this.$router.push({ path: '/editPurchase'});
                }if(doType == 'ok'){
                    this.$confirm('是否确认该采购单?', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
                        ...
                        requestStatusPurchase(sParam).then(res => {
                            if(res.status==400){
                                this.$message({
                                    type:'error',
                                    message:res.msg,
