        ...
        <div class="contentBody"
            ...
            <el-form :inline="true" :model="searchForm" label-width="110px" class="demo-form-inline" label-position="left">
                <el-row :gutter="24">
                    ...
                    <el-col :span="4">
                        <el-form-item label-width="0" style="padding-left: 25px;">
            ...
            <el-table :data="tableData" border style="width: 100%">
                ...
                <el-table-column align="center" header-align='center' prop="orderNo" label="入库单号" width="140"></el-table-column>
                <el-table-column align="center" header-align='center' prop="name" label="入库单名称"></el-table-column>
                <el-table-column align="center" header-align='center' prop="type" label="入库单类型" width="80">
                    <template slot-scope="scope">
                        <div v-if="scope.row.type == 1">采购</div>
                        <div v-if="scope.row.type == 2">直接入库</div>
                        <div v-if="scope.row.type == 3">门店退货</div>
                        <div v-if="scope.row.type == 4">领料单</div>
                    </template>
                </el-table-column>
                <el-table-column align="center" header-align='center' prop="dateTime" label="入库日期" width="140"></el-table-column>
                ...
                <el-table-column align="center" header-align='center' prop="statusName" label="状态" width="80"></el-table-column>
                ...
                <el-table-column align="left" header-align='center' label="操作" width="350">
                    <template slot-scope="scope">
                        ...
                        <el-button style="background-color: #24cbc1;color: #fff;border: none;" icon="el-icon-edit" v-if='scope.row.status == 0' type="primary" size="mini" @click="handleClick(scope.$index, scope.row, 'edit')">编辑</el-button>
...
<script type="text/ecmascript-6">
    import { requestInStockOrderPage, requestDeleteInStockOrder,requestStatusIn,requestInStockOrderCheckStock} from '../../api/api';
    export default {
        name: 'user-manage',
        data() {
            return {
        ...
        methods: {
            ...
            handleSizeChange(val){
                ...
                requestInStockOrderPage(_this.param).then(res => {
            ...
            handleClick(index, row, doType) {
                ...
                if(doType == 'delete'){
                    this.$confirm('真的删除该行数据么, 是否继续?', '提示', {
                        ...
                        requestDeleteInStockOrder(deleteParam).then(res => {
                            this.$message({
                                type: 'success',
                                message: '删除成功!',
                                duration:300,
                    ...
                    sessionStorage.setItem('inId', '');
                    this.$router.push({ path: '/editInStockOrder'});
                }else if(doType == 'edit'){
                    sessionStorage.setItem('inId', row.id);
                    this.$router.push({ path: '/editInStockOrder'});
                }else if(doType == 'detail'){
                    sessionStorage.setItem('inId', row.id);
                    this.$router.push({ path: '/detailInStockOrder'});
                }if(doType == 'ok'){
                    if(row.type == 4){
                        requestInStockOrderCheckStock({id:row.id}).then(res => {
                            if(res.status == 200){
                                this.$confirm('是否确认该入库单?', '提示', {
                                    confirmButtonText: '确定',
                                    cancelButtonText: '取消',
                                    type: 'warning'
                                }).then(() => {
                                    ...
                                    requestStatusIn(sParam).then(res => {
                                        if(res.status==400){
                                            this.$message({
                                                type:'error',
                                                message:res.msg,
