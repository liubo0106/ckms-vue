<template>
    <section>
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>领料单管理</el-breadcrumb-item>
        </el-breadcrumb>
        <div class="contentBody"
            ...
            <el-table :data="tableData" border style="width: 100%">
                ...
                <el-table-column align="center" header-align='center' prop="orderNo" label="领料单号" width="140"></el-table-column>
                ...
                <el-table-column align="center" header-align='center' prop="totalPrice" label="总价" width="80">
                    <template slot-scope="scope">
                       ￥{{scope.row.totalPrice | substr}}
                    </template>
                </el-table-column>
                <el-table-column align="center" header-align='center' prop="statusName" label="状态" width="180"></el-table-column>
                <el-table-column align="center" header-align='center' prop="createUser" label="操作人" width="140"></el-table-column>
                <el-table-column align="left" header-align='center' label="操作" width="350">
                    <template slot-scope="scope">
                        <el-button size="mini" style="background-color: #f37276;color: #fff;border: none;" icon="el-icon-document" @click="handleClick(scope.$index, scope.row, 'detail')">查看</el-button>
                        <el-button v-if='scope.row.status == 0' style="background-color: #24cbc1;color: #fff;border: none;" icon="el-icon-edit" size="mini" type="primary" @click="handleClick(scope.$index, scope.row, 'edit')">编辑</el-button>
                        <el-button v-if='scope.row.status == 0' style="background-color: #fe8d29;color: #fff;border: none;" icon="el-icon-delete" size="mini" type="danger" @click="handleClick(scope.$index, scope.row, 'delete')">删除</el-button>
                        <el-button v-if='scope.row.status == 0' icon="el-icon-success" style="background-color: #6faffd;color: #fff;border: none;" size="mini" type="warning" @click="handleClick(scope.$index, scope.row, 'ok')">确认</el-button>
                    </template>
                </el-table-column>
            </el-table>
            <el-pagination
...
<script type="text/ecmascript-6">
    import { requestMaterialPage, requestDeleteMaterial,requestStatusMaterial,requestPickOrderCheckStock,} from '../../api/api';
    export default {
        name: 'user-manage',
        data() {
            return {
        ...
        methods: {
            ...
            handleSizeChange(val){
                ...
                requestMaterialPage(_this.param).then(res => {
            ...
            handleClick(index, row, doType) {
                ...
                if(doType == 'delete'){
                    this.$confirm('真的删除该行数据么, 是否继续?', '提示', {
                        ...
                        requestDeleteMaterial(deleteParam).then(res => {
                            this.$message({
                                type: 'success',
                                message: '删除成功!',
                                duration:300,
                    ...
                    sessionStorage.setItem('materialId', '');
                    this.$router.push({ path: '/editmaterial'});
                }else if(doType == 'detail'){
                    sessionStorage.setItem('materialId', row.id);
                    this.$router.push({ path: '/detailmaterial'});
                }else if(doType == 'edit'){
                    sessionStorage.setItem('materialId', row.id);
                    this.$router.push({ path: '/editmaterial'});
                }if(doType == 'ok'){
                    requestPickOrderCheckStock({id:row.id}).then(res => {
                        if(res.status == 200){
                            this.$confirm('是否确认该领料单?', '提示', {
                                confirmButtonText: '确定',
                                cancelButtonText: '取消',
                                type: 'warning',
                            }).then(() => {
                                ...
                                requestStatusMaterial(sParam).then(res => {
                                    if(res.status==400){
                                        this.$message({
                                            type:'error',
                                            message:res.msg,
