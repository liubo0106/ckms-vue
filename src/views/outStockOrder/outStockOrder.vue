    ...
    <section>
        ...
        <div class="contentBody"
            ...
            <el-form :inline="true" :model="searchForm" label-width="110px" class="demo-form-inline" label-position="left">
                <el-col :span="24">
                    ...
                    <el-form-item label="搜索状态">
                        <!--<el-input v-model="searchForm.status" :maxlength="200" autocomplete="off" placeholder="请输入" clearable></el-input>-->
            ...
            <el-table :data="tableData" border style="width: 100%">
                ...
                <el-table-column align="center" header-align='center' prop="orderNo" label="出库单号" width="140"></el-table-column>
                <el-table-column align="center" header-align='center' prop="name" label="出库单名称"></el-table-column>
                <el-table-column align="center" header-align='center' prop="type" label="出库单类型"  width="100">
                    <template slot-scope="scope">
                        <div v-if="scope.row.type == 1">门店订单</div>
                        <div v-if="scope.row.type == 2">自主配货</div>
                    </template>
                </el-table-column>
                <el-table-column align="center" header-align='center' prop="dateTime" label="出库日期" width="100"></el-table-column>
                ...
                <el-table-column align="center" header-align='center' prop="statusName" label="状态" width="100"></el-table-column>
...
<script type="text/ecmascript-6">
    import { requestOutStockOrderPage, requestDeleteOutStockOrder,requestStatusOut,requestSaveOutCheckStock} from '../../api/api';
    export default {
        name: 'user-manage',
        data() {
            return {
        ...
        methods: {
            ...
            handleSizeChange(val){
                ...
                requestOutStockOrderPage(_this.param).then(res => {
            ...
            handleClick(index, row, doType) {
                ...
                if(doType == 'delete'){
                    this.$confirm('真的删除该行数据么, 是否继续?', '提示', {
                        ...
                        requestDeleteOutStockOrder(deleteParam).then(res => {
                            this.$message({
                                type: 'success',
                                message: '删除成功!',
                                duration:300,
                    ...
                    sessionStorage.setItem('outId', '');
                    this.$router.push({ path: '/editOutStockOrder'});
                }else if(doType == 'edit'){
                    sessionStorage.setItem('outId', row.id);
                    this.$router.push({ path: '/editOutStockOrder'});
                }else if(doType == 'detail'){
                    sessionStorage.setItem('outId', row.id);
                    this.$router.push({ path: '/detailOutStockOrder'});
                }if(doType == 'ok'){
                    //校验库存
                    let cParam ={
                        id: row.id,
                    };
                    requestSaveOutCheckStock(cParam).then(res => {
                        if(res.status == 200){
                            this.$confirm('是否确认该出库单?', '提示', {
                                confirmButtonText: '确定',
                                cancelButtonText: '取消',
                                type: 'warning'
                            }).then(() => {
                                ...
                                requestStatusOut(sParam).then(res => {
                                    if(res.status==400){
                                        this.$message({
                                            type:'error',
                                            message:res.msg,
        ...
        beforeMount() {
            sessionStorage.removeItem('outId');
            this.getAjaxList();
        }
    }
</script>
