...
<script type="text/ecmascript-6">
    import {requestPurchaseReportPage, requestUrl} from '../../api/api';
    ...
    export default {
        ...
        methods: {
            ...
            handleSizeChange(val){
                ...
                requestPurchaseReportPage(_this.param).then(res => {
                    _this.loading = false;
            ...
            handleClick(index, row, doType) {
                ...
                if(doType == 'export') {
                    ...
                    location.href = requestUrl+'export/purchaseOrderReport'+
                        '?beginDate='+beginDate+
                        '&endDate='+endDate+
                        '&orderNo='+this.searchForm.orderNo+
                        '&name='+this.searchForm.name;
                }else if(doType == 'detail'){
                    sessionStorage.setItem('purchaseReportId', row.id);
                    this.$router.push({ path: '/purchasereportdetail'});
                }
            },
            onSearch() {
                this.param.pageNo = 1;
