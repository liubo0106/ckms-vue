<template>
    <section>
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>报损单管理</el-breadcrumb-item>
        </el-breadcrumb>
        <div class="contentBody"
            ...
            <el-table :data="tableData" border style="width: 100%">
                ...
                <el-table-column align="center" header-align='center' prop="dateTime" label="报损日期" width="140"></el-table-column>
                <el-table-column align="center" header-align='center' prop="orderType" label="报损类别" width="140"></el-table-column>
                ...
                <el-table-column align="left" header-align='center' label="操作" width="440" v-if="isLossManager==1">
                    <template slot-scope="scope">
                        ...
                        <el-button v-if='scope.row.status == 0 || scope.row.status == 2' size="mini" type="warning" icon="el-icon-success" style="background-color: #6faffd;color: #fff;border: none;" @click="handleClick(scope.$index, scope.row, 'ok')">确认</el-button>
                ...
                <el-table-column align="left" header-align='center' label="操作" width="350" v-else>
                    <template slot-scope="scope">
                        <el-button style="background-color: #f37276;color: #fff;border: none;" icon="el-icon-document" size="mini" @click="handleClick(scope.$index, scope.row, 'detail')">查看</el-button>
                        <el-button style="background-color: #24cbc1;color: #fff;border: none;" icon="el-icon-edit" v-if='scope.row.status == 0 || scope.row.status == 2' type="primary" size="mini" @click="handleClick(scope.$index, scope.row, 'edit')">编辑</el-button>
                        <el-button style="background-color: #fe8d29;color: #fff;border: none;" icon="el-icon-delete" v-if='scope.row.status == 0 || scope.row.status == 2' size="mini" type="danger" @click="handleClick(scope.$index, scope.row, 'delete')">删除</el-button>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods: {
                ...
                requestDestroyPage(_this.param).then(res => {
                    ...
                    if(data && data.length>0){
                        ...
                        for(let i=0;i<_this.tableData.length;i++){
                            if(_this.tableData[i].orderType==0){
                                _this.tableData[i].orderType='厨房报损';
                            }
                            if(_this.tableData[i].orderType==1){
                                _this.tableData[i].orderType='库房报损'
                            }
            ...
            handleClick(index, row, doType) {
                ...
                if(doType == 'delete'){
                    ...
                    sessionStorage.setItem('destroyId', '');
                    this.$router.push({ path: '/editdestroy'});
                }else if(doType == 'edit'){
                    sessionStorage.setItem('destroyId', row.id);
                    sessionStorage.setItem('check',row.check);
                    this.$router.push({ path: '/editdestroy'});
                }else if(doType == 'detail'){
                    sessionStorage.setItem('destroyId', row.id);
                    this.$router.push({ path: '/detaildestroy'});
                }
                else if(doType=='error'){
                    this.dialogVisible=true;
                    this.rejectrData.id=row.id;
                }if(doType == 'ok'){
                    this.$confirm('是否确认该报损单?', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
        ...
        beforeMount() {
            sessionStorage.removeItem('destroyDoType');
            sessionStorage.removeItem('destroyId');
            this.getAjaxList();
        }
    }
</script>
