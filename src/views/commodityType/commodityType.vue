                    ...
                    <el-table :data="tableData" border style="width: 100%">
                        ...
                        <el-table-column align="left" header-align='center' label="操作" width="180">
                            <template slot-scope="scope">
                                <el-button size="mini" style="background-color: #24cbc1;color: #fff;border: none;" icon="el-icon-edit" @click="handleClick(scope.$index, scope.row, 'edit')">编辑</el-button>
                                <el-button size="mini"  style="background-color: #fe8d29;color: #fff;border: none;" icon="el-icon-delete" type="danger" @click="handleClick(scope.$index, scope.row, 'delete')">删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                    <!--分页-->
...
<script type="text/ecmascript-6">
    import { requestSearchOrg, requestDeleteOrg, requestSearchDeptPage, requestSaveOrg} from '../../api/api';
    ...
    export default {
        ...
        methods: {
            ...
            handleSizeChange(val){
                ...
                requestSearchDeptPage(_this.param).then(res => {
                    if(res.status == 200){
            ...
            handleClick(index, row, doType) {
                ...
                if(doType == 'delete'){
                    _this.$confirm('真的删除该行数据么, 是否继续?', '提示', {
                        ...
                        requestDeleteOrg(deleteParam).then(res => {
                            if(res.status == 200){
                                _this.$message({
                                    type: 'success',
                                    message: '删除成功!',
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        if( pp[pp.length-1].toString() == this.oldCategory ){
                            ...
                            //表单提交方法更
                            requestSaveOrg(addParam).then(res => {
                                if(res.status == 200){
                                    _this.$message({
                                        type: 'success',
                                        message: '操作成功!',
