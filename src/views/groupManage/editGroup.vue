    ...
    <div class="contentBody">
      ...
      >
        ...
        <el-form-item label="打包分类" prop="deptArray">
          <div slot="label">
            <span class="red-span" v-show="editForm.flag != 'company'" style=" color: #F56C6C;">*</span>打包分类
          </div>
          <template>
            <el-cascader
        ...
        <el-form-item label="单位" prop="standard">
          <el-select v-model="editForm.standard" placeholder="请选择">
            <el-option
              v-for="(item,index) in standardDataDicList"
              :label="item.name"
              :value="item.name"
              v-bind:key="index"
            ></el-option>
        ...
        <el-form-item>
          <el-button
            type="primary"
            style="background-color: #35B178;border-color:#35B178;"
      ...
      <div class="contentBody" v-if="id">
        ...
        <el-table :data="tableData" border style="width: 100%">
          ...
          <el-table-column align="center" header-align="center" prop="costPrice" label="采购单价">
            <template slot-scope="scope">￥{{scope.row.costPrice | substr}}</template>
          </el-table-column>
          <el-table-column
            align="center"
            header-align="center"
          ...
          <el-table-column align="center" header-align="center" label="操作" width="180">
            <template slot-scope="scope">
              ...
              <el-button
                size="mini"
                type="danger"
                @click="handleClickDetail(scope.$index, scope.row, 'delete')"
              >删除</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog :title="dialogTreeTitle" :visible.sync="dialogTreeFormVisible" width="80%" top="5vh">
      <el-container>
        ...
        <el-main>
          <el-form
            :inline="true"
            :model="searchTreeForm"
            label-width="80px"
            class="demo-form-inline"
          >
            <el-form-item
              label="原料编号"
              style="font-family:MicrosoftYaHeiLight;font-weight:bold;color:rgba(102,102,102,1);"
            >
              <el-input
                v-model="searchTreeForm.serialNo "
                :maxlength="200"
                autocomplete="off"
                placeholder="原料编号"
                clearable
            ...
            <el-form-item label-width="0">
              <el-button
                icon="el-icon-search"
                circle
                @click.native.prevent="onSearchTree"
                native-type="submit"
              ></el-button>
            </el-form-item>
          </el-form>
          <el-table :data="tableTreeData" border>
            ...
            >
              <template slot-scope="scope">
                <el-input-number
                  style="width:130px"
                  v-model="tableTreeData[scope.$index].number"
                  :min="0"
                ></el-input-number>
              </template>
            </el-table-column>
            ...
            <el-table-column align="center" header-align="center" label="操作" width="80">
              <template slot-scope="scope">
                <el-button
                  type="primary"
                  size="small"
                  @click="handleClickTreeAdd(scope.$index, scope.row)"
                >添加</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          requestNameEdit(editFormParam).then(res => {
            if (res.status == 300) {
              this.$message({
                type: "error",
                message: res.msg
  ...
  beforeMount() {
    ...
    if (id) {
      ...
      if (this.pathIds && this.pathIds.length > 0) {
        this.editForm.deptArray =
          this.pathIds.length > 0 ? this.pathIds.split(",") : [];
      }
    }
    this.getAjaxProduct();
