        ...
        <div class="contentBody" style="padding-bottom: 0px;">
            <el-form inline label-width="100px" class="demo-ruleForm">
                ...
                <el-form-item  label="折扣">
                    <i class="largeFont">{{orderDetail.discountStr}}</i>
                </el-form-item>
...
<script type="text/ecmascript-6">
    import {requestStoreOrdeInfo,requestStoreOrdeInfoAll} from '../../api/api';
    ...
    export default {
        ...
        methods: {
            getAjaxDetail(){ 
                if(this.type==1){
                    requestStoreOrdeInfoAll({orderNo:this.orderNo}).then(res => {
                        if(res.status == 200){
                            let data = res.data;
                            if (data) {
            ...
            goBack() {
                this.$router.push({ path: '/storeOrde' });
            }
        },
        beforeMount() {
            if(this.orderId){
